...
`gh` is the official GitHub command-line interface. It lets you perform GitHub operations — repositories, pull requests, issues, releases, workflows, etc. — directly from the terminal, without opening a browser.

```
gh <command> <subcommand> [flags]
```

All commands follow the same structure. Flags may be placed before or after arguments. Most read commands accept `--json` for machine-readable output, as well as `--jq` / `--template` for filtering.
...
Authenticate with GitHub. Prompts for account type (GitHub.com or Enterprise), protocol (HTTPS or SSH), and preferred editor. Credentials are stored in the system keychain or in `~/.config/gh/hosts.yml`.
...
Display the current authentication status for all known hosts.
...
Refresh or expand the OAuth token scopes for the current account.
...
Switch to a different account when multiple accounts are authenticated.
...
| 0 | Success |
| 1 | General error |
| 2 | Usage error (incorrect flags, missing arguments) |
| 4 | Authentication required |

Run `gh help exit-codes` for the reference list.
...
| `GH_HOST` | Default host; useful for Enterprise environments |
| `GH_ENTERPRISE_TOKEN` | Token for GitHub Enterprise scripts/automation |
...
| `NO_COLOR` | Disable ANSI colorization |
| `CLICOLOR_FORCE` | Force color even outside a TTY |
...
Set a configuration value. Can be scoped to a specific host with `--host`.
...
| `fork` | Fork the current repository or a specified one |
...
| `delete` | Delete a repository (confirmation required) |
...
| `checks [<number>]` | Display CI checks for a pull request |
| `diff [<number>]` | Display the diff of a pull request |
...
| `lock [<number>]` | Lock the conversation |
| `unlock [<number>]` | Unlock the conversation |
| `revert [<number>]` | Revert a merged pull request |
| `status` | Show pull requests relevant to the current user |
| `update-branch [<number>]` | Update the branch with the base branch |
...
| `comment [<number>]` | Add a comment |
| `delete [<number>]` | Delete an issue (admin rights required) |
...
| `delete-asset <tag> <asset>` | Delete an asset from a release |
| `verify-asset` | Verify the integrity of a downloaded asset |
| `verify` | Verify a release via attestation |
...
| `view [<run-id>]` | Display details and logs for a run |
| `watch [<run-id>]` | Watch a run in real time |
| `rerun [<run-id>]` | Re-run a workflow (or failed jobs only) |
| `cancel [<run-id>]` | Cancel a running workflow |
| `delete [<run-id>]` | Delete a run |
| `download [<run-id>]` | Download artifacts from a run |
...
gh label delete bug
gh label clone owner/source-repo   # copy all labels from another repository
```
...
Interact with GitHub Copilot from the CLI (Copilot subscription required).

```bash
gh copilot suggest "how to squash commits"
...
Inspect rulesets for a repository or organization.
...
Display a dashboard of pull requests, issues, and review requests relevant to the current user, across all repositories.
...
Most list and display commands accept:

| Flag | Description |
|------|-------------|
| `--json <fields>` | JSON output selecting specific fields |
...
# Set the default host for all commands
...
Supported from GitHub Enterprise Server 2.20.
